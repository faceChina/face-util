...
public class TranscodeUtil {
	...
	/**
	 * @Description: 把&'"转义成对应的HTML实体
	 * @param str
	 * @return
	 * @date: 2015年2月7日 下午1:02:05
	 * @author: zyl
	 */
	 ...
	 * @return
	 * @throws Exception
	 * @date 2015年3月18日 下午2:34:00
	 * @author lys
	 */
	public static <T> T transParentToChild(Object obj, Class<T> subClass) {
		try {
			Class<?> superClass = obj.getClass();
			if (!superClass.isAssignableFrom(subClass)) {
				throw new ClassCastException(subClass + " is not sub class of " + superClass);
			}
			...
			Object[] params = null;
			T subObject = subClass.cast(subClass.getConstructor().newInstance(params));
			for (Field field : fields) {
				try {
					Object fieldValue = ReflectUtil.get(obj, field.getName(), superClass);
					if (null == fieldValue) continue;
					ReflectUtil.setValue(subObject, field.getName(), fieldValue, superClass);
				} catch (Exception e) {
					if (log.isDebugEnabled()) {
						log.error(field.getName() + " has no get or set method.", e);
					}
				}
			}
			return subObject;
		} catch (Exception e) {
			throw new RuntimeException("[transParentToChild] has exception.", e);
		}
	}
	 ...
	 * @date 2015年3月18日 下午2:38:38
	 * @author lys
	 */
	public static <T> List<T> transParentToChildList(List<? extends Object> objList,
			Class<T> subClass) {
		...
		for (Object obj : objList) {
			T t = transParentToChild(obj, subClass);
			if (null == t)
				continue;
			list.add(t);
		}
	...
	public static void main(String[] args) {
		List<TranscodeUtil> strList = new ArrayList<TranscodeUtil>();
		transParentToChildList(strList, TranscodeUtil.class);
	}
